...
struct qlog_trigger_optimize_sub_def* qlog_trigger_optimize_sub_init_base(qlog_trigger_optimize_sub_trigger trigger_shrink_func,
  ...
  bzero(qlog_trig_sub, sizeof(struct qlog_trigger_optimize_sub_def));
  ...
  qlog_trig_sub->qlog_trigger_optimize_sub_gate_cnt = 0;
  qlog_trig_sub->qlog_trigger_optimize_sub_pattern_cnt = 0;
...
bool qlog_optimize_shrink_trigger_hadamard_controlled(struct qlog_trigger_optimize_sub_def* qlog_trig_sub, struct qlog_graph_def* qlog_graph, struct qlog_entry_def* qlog_entry) {
#if 0
  if (!qlog_trig_sub || !qlog_graph || !qlog_entry || qlog_graph->qlog_graph_size < 2) {
    return false;
  }
  ...
  if (!(qlog_graph->qlog_graph_used_qubits[qubits[0]])) {
    return false;
  }  

  qlog_node_def* entry_check = qlog_graph->qlog_graph_circuit_track[qubits[0]]; 

  qlog_node_def* controlled_node;

  if (entry_check->qlog_node_prev == NULL ||
      entry_check->qlog_node_prev->qlog_node_entry == NULL) {
    return false;
  }
  ...
  if (controlled_node->qlog_node_prev == NULL ||
     controlled_node->qlog_node_next == NULL ||
     !(controlled_node->qlog_node_prev->qlog_node_entry->qlog_entry_gate == GLOBAL_GATE_HADAMARD) ||
...
bool qlog_optimize_shrink_trigger_duplicate_gates(struct qlog_trigger_optimize_sub_def* qlog_trig_sub, struct qlog_graph_def* qlog_graph, struct qlog_entry_def* qlog_entry) {
  ...
  for (uint16_t i = 0; i < qlog_entry->qlog_entry_qubit_cnt; ++i) {
    if (qlog_graph->qlog_graph_row_size[qlog_entry_qubits[i]] == 0 ||
        qlog_graph->qlog_graph_circuit_track[qlog_entry_qubits[i]] == NULL ||
        qlog_graph->qlog_graph_circuit_track[qlog_entry_qubits[i]]->qlog_node_id == 0 ||
        qlog_graph->qlog_graph_circuit_track[qlog_entry_qubits[i]]->qlog_node_entry == NULL) {
      return false;
    }

    qlog_entry_def* prev_entry = qlog_graph->qlog_graph_circuit_track[qlog_entry_qubits[i]]->qlog_node_entry;

    if (prev_entry == NULL || prev_entry->qlog_entry_prev == NULL) {
      return false;
    }
