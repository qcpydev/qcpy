...
const int qlog_expand_gate_count = sizeof(qlog_expand_gates) / sizeof(qlog_expand_gates[0]);
...
static bool qlog_expand_previous_gate_is_equal(qlog_node_t* qlog_node)
{
    ...
    assert(qlog_node->qlog_entry->controlled_bitmask && qlog_node->qlog_entry->target_bitmask);
